...
//
// Each member is run by resetting this worker's own arrays between runs, which
// is what a real engine instance would do anyway. withWorld() is used where it
// exists because it is tidier than clearing by hand, but the isolation that
// matters here comes from the realm, not from withWorld.
// =============================================================================
...
function restore(P, payload, perturb) {
  ...
  for (const name of P.WORLD_LISTS) P[name].length = 0;
...
self.onmessage = async e => {
  ...
  try {
    ...
    self.postMessage({
      ...
      // Proof that this realm has its own module instance: a counter that
      // started at zero here while the page's copy is in the thousands.
