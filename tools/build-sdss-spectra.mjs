...
export const SPECTRA = ${JSON.stringify(spectra, null, 2)};
...
async function structuralCheck() {
  ...
  for (const src of SOURCES) {
    const spec = SPECTRA?.[src.id];
    if (!spec) {
      problems.push(`${src.id}: missing`);
      continue;
    }
    for (const field of [
      'specObjID',
...
try {
  if (check) {
    ...
    if ((await readFile(OUT, 'utf8')) !== next) {
      console.error(
        'js/data/spectra/sdssSpectra.js is not what tools/build-sdss-spectra.mjs\n' +
          'would write. Run `npm run spectra:data` and commit the result.'
      );
      process.exit(1);
    }
    console.log(
