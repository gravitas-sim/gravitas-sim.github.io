 ...
 *
 * It used to be a file name, and a file name proves nothing. Then it was a tag,
 * and a tag proved less than it looked like it proved: an independent audit
 * found thirteen of the twenty-two ids carried by more than one test and
 * `ce.keplers-laws` by three, because two files wrote the tag into a generated
 * title and every lesson their loop covered picked up a binding for free.
 *
 * The rules now live in tools/acceptance-bindings.mjs, which is a pure function
 * so that tests/acceptanceBindings.test.js can drive it with a tree built to
 * fail. This gathers what that function needs: every spec file, and the list of
 * what Playwright would really collect.
 *
 * Listing is not spawnable from Jest - Playwright detects a Jest process and
...
async function checkAcceptanceBindings(acceptance, lessons) {
  ...
  for (const name of names) {
    ...
    const text = await readFile(resolve(dir, name), 'utf8').catch(() => null);
  ...
  return auditBindings({
    ...
    lessons,
    sources,
    collected: parseCollected(listing),
  });
}
...
function provenanceModule(acceptance) {
  const rows = Object.entries(acceptance?.lessons || {})
    ...
    .filter(([, v]) => v)
    .sort(([a], [b]) => (a < b ? -1 : 1));
  // Quoted only where a quote is needed, which is what prettier does with its
  // default quoteProps and therefore what the format check demands of a file
  // it does not know is generated. Nearly every lesson id has a hyphen and
  // needs quoting; "tides" does not, and an unconditional quote there failed
  // the gate on a file no human wrote.
  const key = id => (/^[A-Za-z_$][A-Za-z0-9_$]*$/.test(id) ? id : `'${id}'`);
  const body = rows.map(([id, v]) => `  ${key(id)}: '${v}',`).join('\n');
