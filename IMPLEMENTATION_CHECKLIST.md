...
> **Historical record — superseded.** This was the live working file for two
> work packages, and both of them shipped. Package A became the investigation
> *Listening to Spacetime*; package B became *A Universe of Stars* and *Lives
> of Stars*, and all three are in the manifest and in the lesson browser. The
> unchecked boxes, the "not started" notes and the deferred checks below are
> the state of the work at the time each line was written, and are deliberately
> not updated — rewriting a work log to agree with its outcome destroys the
> only thing a work log is for.
>
> This file is not product documentation and nothing links to it. For what the
> software does now, start at [`README.md`](README.md); for the lessons, the
> manifest is generated by `npm run manifest`; for what has been verified,
> [`PHYSICS_VALIDATION.md`](PHYSICS_VALIDATION.md) and `npm run release:check`.
