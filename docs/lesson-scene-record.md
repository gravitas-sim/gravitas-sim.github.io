...
Across 23 investigations and 654 steps: 607 steps
declare a scene, 157 let a reader change one, 163 read the
running integration, 225 show a prescribed model result, 146 draw
on a stored dataset, 104 put a self-contained panel beside the text,
110 keep something, and 26 are prose.
Of 82 prediction loops, 82 are returned to,
82 produce a measurement before they are answered, and
...
- **Stored data:** Published exoplanet system parameters (observation); TRAPPIST-1 system parameters (observation); GW150914 strain, as published (observation); MIST v1.2 evolutionary tracks (computed-grid); 51 Pegasi radial velocities, as published (observation).
...
- **Stored data:** MIST v1.2 evolutionary tracks (computed-grid); Published exoplanet system parameters (observation); TRAPPIST-1 system parameters (observation); GW150914 strain, as published (observation); 51 Pegasi radial velocities, as published (observation).
- **Evidence kept:** notebook-fields, instrument-capture.
...
## Somebody Else's Star (`replicating-51-peg`)

- **Steps:** 18. Scene declared on 0; a reader can change something on 0; 0 declare a scene nobody on that step can touch.
- **Where the numbers come from:** 0 live engine, 0 prescribed model, 7 stored dataset, 0 self-contained panel.
- **What students do:** read, commit-a-prediction, tick-a-checklist, record-values, choose-an-option, answer-in-words.
- **Objects the steps declare:** none.
- **Instruments:** none.
- **Stored data:** none.
- **Evidence kept:** notebook-fields.
- **Prediction loops:** before-you-look → the-shape-of-the-run, write-down-the-fit, the-fit-is-good-but, write-down-the-interval → what-a-gap-does.
- **Recovery:** reset: rebuilds the scenario on arrival; missed moment: nothing stops the run here, so a reading that changes with time has to be caught by hand; ambiguous result: the check says in words how close is close enough.
- **Scene rebuilds:** 1 step(s) on arrival (the-claim).
- **Accepted central experiment:** not yet reviewed.
