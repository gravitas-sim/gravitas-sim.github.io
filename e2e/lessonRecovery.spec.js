...
test.describe('the object list names what a step means', () => {
  ...
  for (const [id, expected] of [
    ...
    test(`${id}: every intended body is listed and selectable @covers:ce.${id}`, async ({
      page,
      app,
    }) => {
...
test.describe('an instrument and the scene describe the same thing', () => {
  ...
  test('transit-photometry: the phase readout tracks the planet across the disc @covers:ce.transit-photometry', async ({
    page,
    app,
  }) => {
    test.slow();
  ...
  test('gravity-assist: a rebuilt scene marks the previous run stale @covers:ce.gravity-assist', async ({
    page,
    app,
  }) => {
    test.slow();
