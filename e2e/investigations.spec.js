...
test.describe('opening a lesson', () => {
  ...
  test('a lesson sets up its own scenario and keeps the sim alive @covers:ce.weighing-stars', async ({
    page,
    app,
  }) => {
...
test.describe('advancing through representative step types', () => {
  test(
    'walks the first several steps, meeting an instrument on the way @covers:ce.goldilocks-question',
    { tag: '@cross-browser' },
    async ({ page, app }) => {
      await openLesson(page, app);
...
test.describe('persistence', () => {
  ...
  test('answers are remembered, not just the step number @covers:ce.tides', async ({
    page,
    app,
  }) => {
    await openLesson(page, app);
...
test.describe('the student report', () => {
  test('the finish dialog opens and can produce a report @covers:ce.missing-mass', async ({
    page,
    app,
  }) => {
