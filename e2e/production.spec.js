...
// Read in Node, not in the page: this spec cannot import application modules
...
test.describe('the built site', () => {
  ...
  test('the teaching page ships, with its own stylesheet and bundle', async ({
    ...
    const failures = watchRequests(page);
    const response = await page.goto('/teaching/', { waitUntil: 'load' });
    expect(response.status()).toBe(200);

    // Its stylesheet and its module are built by separate steps from the
    // application's own, precisely so that neither reaches the simulation's
    // start-up path - which means either can be left out of dist/ without
    // anything else noticing.
    for (const asset of ['/css/teaching.css', '/js/teachingPage.js']) {
    ...
    // The demonstrations are generated by that module, so their presence is
    // the evidence it ran rather than 404'd.
    await expect(page.locator('#teachDemos article')).toHaveCount(DEMOS.length);
    await expect(page.locator('#teachGlance li').first()).not.toBeEmpty();

    // And one figure actually opens. A demonstration is `?embed=1` plus a share
    // fragment against the built application, which is a different code path
    // from the sources and the one a copy step can break without the page
    // itself looking any different.
    const card = page.locator('#teachDemos article').first();
    await card.getByRole('button').click();
    await expect(
      card.frameLocator('iframe').locator('#simulationCanvas')
