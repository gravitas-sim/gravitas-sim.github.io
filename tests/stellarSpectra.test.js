...
describe('provenance', () => {
  ...
  test('the recorded bin factor is the one the grid actually has', () => {
    ...
    expect(GRID.count * Math.round(factor)).toBeLessThanOrEqual(
      Math.min(...SPECTRUM_IDS.map(id => SPECTRA[id].sourceSamples))
    );
  });
...
describe('checksums', () => {
  test('every payload is the payload its checksum names', () => {
    for (const id of SPECTRUM_IDS) {
      const s = SPECTRA[id];
      const digest = createHash('sha256')
        .update(Buffer.from(s.data, 'base64'))
        .digest('hex');
      expect(digest).toBe(s.payloadSha256);
    }
  });
...
describe('a published index, defined elsewhere, agrees', () => {
  ...
  test('the recorded TiO5 on each spectrum is that index', () => {
    for (const id of SPECTRUM_IDS) {
      expect(tioFiveIndex(decodeSpectrum(id))).toBeCloseTo(SPECTRA[id].tio5, 3);
    }
  });
});
...
describe('nothing reaches the network at run time', () => {
  ...
  test('the archive appears only as recorded provenance, never as a load', () => {
    ...
    // every one of them is inside a string in PROVENANCE or SPECTRA and none
