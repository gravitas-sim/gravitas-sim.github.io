...
// =============================================================================
// A deferred catalog must not stand in for a base one
// -----------------------------------------------------------------------------
// ensureDeferredMessages() registers BOTH locales, so a visitor reading English
// who opens any deferred tool ends up with a Spanish catalog containing only
// that tool's strings. loadLocale() then sees a truthy CATALOGS.es and returns
...
/** A key that only the base Spanish catalog has. */
const BASE_KEY = 'observing.session.newWorld';
/** A key that only the deferred catalog has. */
const DEFERRED_KEY = 'cr3bp.title';

describe('registering deferred strings before the base catalog loads', () => {
  test('the base catalog still loads afterwards', async () => {
    ...
    // already in memory.
    const catalog = await loadLocale('es');

    // The base file really was fetched, rather than the handful of registered
    // strings being mistaken for it.
    expect(catalog[BASE_KEY]).toBe(ES[BASE_KEY]);
    // Registered strings live beside the catalog rather than inside it, so
    // the check that matters is the one a caller makes: both resolve.
    expect(catalog[DEFERRED_KEY]).toBeUndefined();
  });
  ...
  test('isLocaleLoaded reports the base, not a handful of deferred keys', async () => {
    ...
    // order: a locale is "loaded" when its base catalog is in memory.
...
// =============================================================================
// Loading the deferred catalog exactly once, for everybody
// =============================================================================
