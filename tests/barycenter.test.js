...
// -----------------------------------------------------------------------------
// js/lesson/barycenter.js is pure, so this needs no simulation and no DOM: the
...
// barycenterOf() is a definition and holds for anything; circularBinary() is a
...
import {
  barycenterOf,
  circularBinary,
  distancesFromBarycenter,
  splitByArms,
  totalMassFromOrbit,
} from '../js/lesson/barycenter.js';
...
describe('the balance point is a definition, not a model', () => {
  test('equal masses balance halfway between them', () => {
    const c = barycenterOf([at(-100, 0, 5), at(100, 0, 5)]);
  ...
  test('nothing to average over is null, not the origin', () => {
    ...
    expect(barycenterOf([])).toBeNull();
    expect(barycenterOf(null)).toBeNull();
    expect(barycenterOf([at(0, 0, -3)])).toBeNull();
  });

  test('the arm lengths are the inverse of the mass ratio', () => {
    const arms = distancesFromBarycenter([at(0, 0, 3), at(400, 0, 1)]);
...
describe('splitting a measured total between the two', () => {
  ...
  test('the split of a modeled pair returns the masses it was built from', () => {
    const ic = circularBinary({ m1: 3, m2: 1, separation: 400, G: 1 });
