...
const GW_SANDBOX = {
  scenario: 'GW150914',
  seed: 'listening-to-spacetime',
  camera: { zoom: 1.7, pan: { x: 0, y: 0 } },
  paused: false,
};

/** The lab, in the configuration a step wants it. */
const lab = (extra = {}) => ({ id: 'gw-lab', ...extra });
...
const LISTENING_TO_SPACETIME = {
  ...
  steps: [
    ...
    // -----------------------------------------------------------------------
    {
      ...
      body: `In the panel is a recording of something. Nobody has told you what.
             \n\nThe top plot is the strength of a signal against time, and the
             bottom one is its frequency. Press <strong>Play / pause</strong>
      ...
      tool: lab({
        ...
        presets: false,
        note: 'A recorded signal. The upper plot is its strength, the lower one its frequency. Nothing here says yet what made it.',
      }),
      tip: 'The slider marked "Position in the signal" is a playhead: drag it to move through the recording, or press Play / pause to let it run.',
    },
    {
      ...
      body: `Here is the source. Two compact objects, circling each other,
             ...
             sandbox. Its two black holes spiral in because the simulation
             multiplies their orbits down by a small factor every step, chosen
             so that a merger happens while somebody is watching. It is not a
             calculation of anything. It is there because it looks like what is
             happening.
    ...
    },
    {
      ...
      tip: 'Separation is given in Schwarzschild radii of the combined mass. Under about three of them there is no stable circular orbit left, which is where the model stops.',
    },
    {
    ...
    },
    {
      ...
      body: `This comparison only works if nothing else moves, so the panel has
             ...
             quieter. If you use the sound, the gain is pinned the same way.
             \n\nSet the distance to 400 Mpc and read the peak strain. Then 800.
             Then 1600. Read the frequency at the end each time as well.`,
      fields: [
        { id: 'h_400', label: 'Peak strain at 400 Mpc', unit: '' },
        { id: 'h_800', label: 'Peak strain at 800 Mpc', unit: '' },
        { id: 'h_1600', label: 'Peak strain at 1600 Mpc', unit: '' },
        {
          id: 'ratio_1',
          label: '400 Mpc peak ÷ 800 Mpc peak',
          unit: '',
      ...
      validate: v => {
        if (![v.h_400, v.h_800, v.h_1600].every(Number.isFinite)) {
          return {
            level: 'warn',
            message:
              'Read the peak strain at all three distances. The readout gives it on the line marked "Strain now" when the playhead is at the end.',
          };
        }
      ...
      tool: lab({
        ...
        hide: ['m1', 'm2', 'inclination'],
        presets: false,
        note: 'The vertical scale is pinned for this step, so the three distances can be compared on one axis.',
      }),
    ...
    },
    {
      ...
      body: `Look at the readout for the black-hole preset again, at two lines
             in particular.
             \n\n<strong>Where it stops</strong> says 67.6 Hz. That is where two
             objects of that combined mass run out of stable circular orbits;
             past it they fall together, and this model has nothing to say about
             what happens next.
