...
export const INSTRUCTOR_CONTENT = {
  ...
  'hohmann-transfer': {
    ...
    misconceptions: [
      {
        ...
        response:
          'A radial burn does raise the far side - just very inefficiently, and it drops the near side by nearly as much, so what you buy is eccentricity rather than size. Have students preview a radial burn at the third screen and read two rows together: the angular momentum does not move (a radial push exerts no torque) while the apoapsis rises anyway (it still adds energy). Those two rows side by side are also the answer to the tempting half-truth below.',
      },
      {
        claim:
          'Angular momentum is what decides how far out an orbit reaches, so a burn that does not change it cannot raise the apoapsis.',
        response:
          'Half right, and the wrong half is the one students carry away. Where an orbit turns around depends on its energy and its angular momentum together - the turning points are the two roots of the effective-potential equation, and both quantities appear in it. Fix the angular momentum and add energy, as a radial burn does, and the apoapsis goes up while the periapsis comes down: same angular momentum, bigger and much more eccentric orbit. What makes the transverse burn the right tool is not that it is the only one that can raise the apoapsis but that it is far better at it: spent along the direction of travel it buys the most energy per unit of Δv, and it raises the angular momentum too, so the orbit grows instead of stretching. The third screen shows both halves at once.',
      },
      {
    ...
    expectations: {
      ...
      15: 'The coast takes about 423 simulated days and the speed falls from 35.6 km/s to 14.2 as the spacecraft climbs. At normal speed this is a long wait; the transport control is the intended route.',
      16: 'The top of the arc reads about 250 simulation units, which is 2.5 AU, and the bottom still reads about 100, which is 1 AU. The bottom is the number that answers the previous screen: the first burn moved the far side of the orbit and left the near side exactly where it was, so with no second burn the spacecraft returns. A student whose bottom figure has changed has applied a burn somewhere other than at periapsis.',
  ...
  'binary-star-planets': {
    ...
    misconceptions: [
      ...
      },
      {
        ...
        response:
          'Step 22. Students who have internalized "close is safe" apply it outside the binary and predict that bringing the planet in is safe. The direction reverses because what makes a region safe is one source dominating, and outside the pair that means being far enough away to see them as one.',
      },
      {
        claim:
  ...
  'detect-this-planet': {
    ...
    modelNotes: `The star and planet are integrated as a Newtonian two-body system in a plane, and the
      ...
      phase and the velocity a spectrograph would report at that instant. That is the link
      between the planner, which works in days since the run began, and the orbit, which is a
      place the star is in - without it a student can read a coverage figure without ever
      seeing that a phase is somewhere the star has been.`,
  ...
  'weighing-stars': {
    ...
    modelNotes: `Two different things are on screen at once and the lesson depends on students
      ...
      the see-saw screens exist for.

      The panel diagrams are analytic: circular two-body orbits evaluated in closed form, with the
      stopwatch reading against the panel’s own clock. The Sirius panel solves Kepler’s equation so
      the plotted epochs are correctly spaced in time, and its dots are real astrometry rather than
      anything this application computed - screen 31 says so to the student.

      Two restrictions bite on the result. The staged orbits are circular and seen face-on. A real
      binary is eccentric, so the "a" in the formula is the semi-major axis and not whatever
      separation you happen to catch; and a real orbit is tilted, so a measured separation is a
      projection of the true one. Both travel with the notebook entry as limitations, and screen 32
    ...
    expectations: {
      ...
      24: 'One lap takes about 4 years. Capture this orbit records the separation, both arm lengths and the timing into the notebook, and marks the period as student-timed - a period taken from the model instead is recorded as a prediction, which is the distinction that entry exists to keep.',
    ...
    features: [
      {
        ...
        text: 'The Sun, Earth and Mars at their real distances, periods and masses, and nothing else. The full Solar System draws the same loops, but with fifty asteroids and ten comets looping at once it is a picture rather than a measurement.',
      },
      {
        name: 'The reference-frame switcher',
        text: 'In Tools > Frame and in the object inspector under Overlays. Choosing a body puts it at rest and re-expresses every position and every trail around it.',
      },
      {
        name: 'Frame-relative readouts',
        text: 'Frame-relative readouts in the inspector: Distance from Earth and Direction from Earth appear whenever a frame is active, alongside the unchanged world-frame Velocity row. The direction in degrees is the number steps 16 and 31 are read from.',
      },
      {
        name: 'A trail long enough to close the loop',
        text: 'The scenario holds about 110 days of history, a little longer than a whole retrograde episode.',
      },
    ],
    misconceptions: [
      {
  ...
  'goldilocks-question': {
    ...
    modelNotes: `The habitable-zone boundaries come from the Kopparapu et al. (2013) prescription with
      ...
      insolation measurement clean, and the real eccentricities of Earth and Mars would change
      nothing about the argument. That leaves the screens about a planet whose distance changes
      round its year with nothing live to point at, so screen 27 stands up one of its own -
      Wanderer, on a genuine ellipse of eccentricity 0.45, moved by the same gravity solver as
      everything else. The screen says so, and the scene is named again on screen 29 when the
      subject changes to TRAPPIST-1.
  ...
  'when-orbits-lock': {
    ...
    expectations: {
      ...
      28: 'A center near 296 degrees, an amplitude near 24 and a period near 13 Jupiter years against a linearized prediction of 12.47. The validation catches a student who has read L4 instead of L5, which is the common slip, and one who has recorded Jupiter’s own period instead of the libration period.',
  ...
  'lives-of-stars': {
    ...
    features: [
      {
        name: 'One age drives every view',
        text: 'Temperature, radius, luminosity, color, current mass and phase all come from the same position on the same track, so the picture and the numbers cannot disagree. Changing the age changes all of them together, and there is nowhere for a stale value to hide.',
      },
      {
        name: 'Nothing depends on waiting',
        text: 'Every phase is reachable with one button, the age is both a slider and a number, and no screen requires an animation to run to completion. Reduced motion turns autoplay off and draws the cloud and the explosion at a fixed moment; everything stays reachable stepwise.',
      },
      {
        name: 'Three clocks, and only one of them is a lifetime',
        text: 'The playhead button cycles three pacings and the readout names which is in force every time. Phase pacing runs along the track’s stored samples so that brief stages are inspectable. Time pacing is logarithmic in age. Proportional pacing is the honest one: the position IS the fraction of the life, which puts eighty-six per cent of a solar-mass playhead on the main sequence and squeezes the whole thermally-pulsing AGB into a hundredth of a per cent. Show that one at least once. Under the other two the readout says, in as many words, that the position is not a fraction of a life - the phase pacing gives the thermally-pulsing AGB nearly half the playhead, an exaggeration of about three thousand eight hundred, and the panel prints that ratio.',
      },
      {
      ...
      },
      {
        ...
        text: 'One screen offers "Freeze this moment", which leaves a copy of the star as it is on the canvas, labeled with the age it was taken at, while the original goes on evolving beside it. The copy is model-owned and nothing writes to it again, so "then and now" is two objects a student can click rather than a number they have to remember. Copies are dropped when the step changes, so a class that freezes several will not carry them forward.',
      },
      {
        name: 'Endpoints carry their sources',
        text: 'Every endpoint states whether it came from the track or from a published prescription, and where it is quoted it names the paper. The 20 solar-mass case is left as "a neutron star or a black hole" because the sources disagree, and that disagreement is presented as the finding rather than as a defect.',
      },
      {
        name: 'Nothing accumulates',
        text: 'The path on the diagram is recomputed between the start and the playhead rather than appended to, and the shells of lost material are seeded on the track. Seeking backwards shortens the path and removes them; replaying draws the same picture. There are no leftover remnants or duplicated events after a scrub.',
      },
    ],
    expectations: {
  ...
  'a-universe-of-stars': {
    ...
    overview: `Students meet three unlabeled stars, guess what the picture can tell them, and then
      ...
      than a picture of different ones. Two things follow that are worth saying to a class. The
      stars do not move and are not orbiting each other - they are a display, and the stellar
    ...
    features: [
      ...
      },
      {
        ...
        text: 'Four hundred stars are drawn from the mass function; 351 of them sit on tracks the model can evaluate and are the ones placed; some number of those pass the brightness cut; and what stands on the canvas is a bounded sample of the placed ones. The readout names all four apart because conflating any two of them is the misconception the screens exist to break - "351 of 400 were modeled" is not "16 of 351 are bright", and neither is a statement about four hundred stars. Expect at least one student to report the canvas count as the population.',
      },
      {
        name: 'Moving the cut moves both views, and re-rolls nothing',
        text: 'The threshold slider filters one generated population. Raise it and stars leave the canvas and the plot together; lower it and exactly those stars come back, in the same places, because the sample was never regenerated. That is worth doing slowly in front of the room: the claim "the same four hundred stars, not a new sample" is the whole basis of comparing the two counts, and a student who suspects the computer of drawing a new sample each time has no reason to believe the second count means anything.',
      },
      {
        name: 'The age slider has two pacings',
        text: 'Paced by time it is honest about how long phases last, and useless for reaching them - the whole red-giant branch of a solar-mass star is two thousandths of its travel. Paced by phase every stage is reachable and the handle is no longer a clock. The readout says which is in force, and screens 20 and 23 deliberately use different ones.',
      },
      {
        name: 'True sizes, or fitted sizes, never both',
        text: 'On the comparison stage "true relative sizes" uses one scale, so a dwarf beside a supergiant is genuinely sub-pixel and is drawn as a labeled marker rather than inflated. "Fit each star" magnifies each into its own box and prints the magnification under each one. The caption states which mode is in force.',
      },
      {
        name: 'The population is synthetic and says so',
        text: 'Four hundred stars from a Kroupa (2001) mass function and a constant star-formation rate over ten billion years, reproducible from a seed. Stars that had already left the main sequence are dropped rather than guessed at, and the count of dropped stars is reported. It is not a survey and the readout says what it omits: dust, binaries, and any composition but solar.',
      },
    ],
    expectations: {
      ...
      21: 'About 47,600 K, 1.6 solar luminosities and 0.018 solar radii - roughly twice the radius of the Earth, holding 0.54 solar masses.',
      24: 'About 1,140,000, 9,880 and 8.7 million years for 0.2, 1 and 20 solar masses. Students often mis-key the trillion; the validation accepts billions and says so.',
      25: '227 M, 93 K, 21 G out of 351 placed. Not one O and not one B: the sample drew a few and they had already left the main sequence.',
      27: 'Sixteen stars kept, of which 8 F, 6 G, 2 A, and no K or M at all.',
      29: 'Any pair where the hotter star is the fainter, or where the cooler star exceeds ten solar radii. The end of the 1 solar-mass track and the end of the 20 solar-mass track each supply one in a single click.',
    },
    misconceptions: [
      {
        claim:
  ...
  'what-is-a-gravitational-wave': {
    topic:
      'What a gravitational wave is: what emits one, what it does on arrival, and how an instrument notices',
    difficulty:
    ...
    overview: `Twenty-four short screens, built around two objects standing on the main canvas rather
      ...
      separation shrink while the plot beside it rises.
      \n\nThe spine is a sequence of three questions. What has to be happening at the source -
      answered on screen 4, where a pulsating sphere is the instructive wrong answer. What arrives
      ...
      say which is which.`,
    priorKnowledge: [
      'That planets and moons go round things',
      'That light takes time to travel',
      'Nothing else. Strain, frequency and the speed of light are all introduced here',
    ],
    keyConcepts: [
      {
        heading:
      ...
      },
      {
        ...
        body: 'Screen 4 offers a pulsating sphere as the deliberate wrong answer, and it catches most classes. The sphere really is moving - every part of it accelerating in and out - and it radiates nothing, because from outside nothing about it changes. The rule is not "moving mass radiates" but "the arrangement of the mass has to look different from outside, and has to keep changing". A perfectly round spinning star is the same trap in another costume.',
      },
      {
        heading: 'The effect is transverse, and it alternates',
        body: 'Screens 9 to 11 build this in three moves: stretched one way means squeezed at right angles at the same instant, half a cycle later the two swap, and the markers themselves go nowhere. That last point is the one that makes a detector make sense - what changes is the distance between things, not the position of anything - and it is also the point students most often skip past, because a ring turning into an oval looks like the ring is being blown outwards.',
      },
      {
      ...
      },
      {
        ...
        body: 'Screen 6 reasons it out and screen 16 has students count it on the canvas rather than be told. The cause is a symmetry: swap two identical objects and the arrangement is the one you started with, so half a turn is a full wave cycle. This factor of two is the origin of most of the arithmetic confusion in the subject later on, and a class that has counted it once does not lose it.',
      },
      {
        heading: 'An interferometer measures a difference, not a length',
    ...
    features: [
      {
        ...
        text: 'Screens 3 to 5 share one control — "What is emitting" — with three settings: one mass sitting still, a sphere pulsing in and out, and a binary. Each one restages the main canvas, so the thing being discussed is the thing on screen. Only the third emits, and the readout says why for each. This used to be three screens of prose with a binary orbiting behind all of them, including the one asking the reader to picture a single static mass.',
      },
      {
        name: 'The rings are propagation, not a field',
        text: 'What the canvas draws is one crest per cycle of the wave, leaving the source and traveling outward, with where each crest sits computed from the model’s own emission history. So pausing holds the pattern, seeking back shows the earlier one, and restarting clears it. What it is NOT: a metric perturbation, a potential, a shell of matter, or a radiation pattern — the rings are circles and a real quadrupole does not radiate isotropically. The propagation speed on screen is a display choice and the readout says so.',
      },
      {
      ...
      },
      {
        ...
        text: 'A real strain of one part in 10²¹ is invisible at any honest scale, so every picture of one is amplified. The control says by how much and the readout prints the factor beside the real numbers. It changes the drawing only — the strain, the arm readings and the meters figure are all the unamplified values.',
      },
      {
        name: 'The binary is on the main canvas',
        text: 'The binary is on the main canvas and selectable throughout; the instrument beside the text is a second view of the same objects',
      },
      {
        name: 'Controls arrive one at a time',
        text: 'Controls are introduced one at a time - the mass and distance sliders do not appear until the screens that use them',
      },
      {
        name: 'Sound is offered, never required',
        text: 'Every screen is completable with the sound off; Listen is offered three times and required never',
      },
    ],
    misconceptions: [
      {
        claim: 'Anything that moves emits gravitational waves.',
        response:
          'The pulsating sphere on screen 4 is the counterexample, and it is worth drawing on the board: every part of it is accelerating and it emits nothing, because from outside it never looks any different. The rule is a changing, non-spherical arrangement of mass. A perfectly round spinning star emits nothing either.',
      },
      {
        claim: 'The wave pushes things outwards as it passes.',
        response:
          'Screen 11. Have students follow one marker for a whole cycle and see it return. What travels is the disturbance; what changes is the distance between things, and that distinction is the entire basis of how a detector works.',
      },
      {
        claim: 'We heard the black holes.',
        response:
          'Screen 14. The application converts a changing signal into a sound on the student\u2019s own machine; a microphone at the source would hear nothing, because sound needs a material and there is none. Ask what the microphone would be sitting in.',
      },
      {
        claim: 'The ring picture shows how big the effect is.',
        response:
          'It shows the shape of the effect at an enormous exaggeration, and screen 12 says by how much. The real change would move a marker by far less than the width of one atom of the screen. Students who leave with the picture but not the scale have half the idea.',
      },
      {
    ...
    expectations: {
      ...
      24: 'The two arms are drawn over the marker ring and the readout gives both as numbers with opposite signs, plus their difference and what that difference is in meters on four-kilometer arms. This screen is edge-on deliberately — see the note on polarization below.',
      ...
      10: 'The ring becoming an oval and turning over. Expect at least one student to think the ring is the orbit seen from above; the body text says it is not, and it is worth repeating.',
      12: 'Two different numbers - whichever direction they recorded first, the other one half a cycle later. The value that matters is not which is which but that they differ; a pair of identical answers means the scrubber was moved a whole cycle rather than half, and the validator says so. Worth adding out loud: between the two stops there is an instant when the ring is a perfect circle and nothing is happening to it, which is not a gap in the wave but the moment it passes through zero.',
      13: 'A marker that returns to where it started. If it does not, the student is watching the whole ring rather than one dot.',
      14: 'Nothing measurable - this screen is a statement, not a task. The number to leave them with is one part in 10²¹, and the atom-across-an-astronomical-unit comparison is the one most students remember.',
      15: 'A practice answer of 0.0001 and a real one around 10⁻²¹. The common error is reading "Strain now", which passes through zero twice a cycle; the validator rejects anything above 10⁻¹⁵ and says why.',
    ...
    extensions: [
      'Have students estimate the strain from a source they invent - two people running round each other - and find out how many orders of magnitude short of measurable it is. The answer is comically large and makes the point better than any statement.',
      'Screen 21 changes the distance. Ask what would happen to the *duration* in band if the masses changed instead, then have them check it on screen 20 with the neutron-star preset.',
      'For a class with some trigonometry: the wave frequency is twice the orbital frequency for equal masses on a circle. Ask what would have to be different about the source for that to fail.',
    ...
    modelNotes: `Everything quantitative here comes from the same leading-order inspiral model the
      ...
      \n\nWhat is illustration, stated plainly because students will ask. The two discs on the
      canvas are fixed-size markers, not sized bodies: their separation is to scale in Schwarzschild
      radii of the total mass and their diameters are not to scale at all, because a horizon and a
      neutron-star surface are not the same kind of quantity. The rings leaving the pair mark wave
    ...
    teachingNotes: [
      'Screen 4 reliably takes longer than expected. The pulsating sphere is genuinely counter-intuitive and worth arguing about, and the argument is the point of the screen rather than a detour from it.',
      'Screen 6 is the other one that overruns. The half-orbit answer needs to be seen rather than asserted — have a student step the playhead themselves at the front.',
      'If time is short, screens 15 and 20 can be dropped without breaking the thread. Screens 4, 11, 12 and 19 cannot: they carry the four ideas that make the sequel possible.',
      'For a class going straight on to Listening to Spacetime, the natural break is after screen 25, which is where the two recorded detections are shown. The closing screens make a good opener for the second session: screen 26 designs one small experiment, screen 27 asks students to tell the whole story in their own words before anybody adds to it, and screen 28 is the summary.',
    ],
  },
  'listening-to-spacetime': {
    ...
    features: [
      {
        name: 'The canvas follows the preset, components and all',
        text: 'Selecting "two neutron stars" changes the masses, the distance, the waveform, the separation on the canvas AND what the two objects are drawn as — two neutron stars rather than two black holes with light masses. That last part used to be missing, and it mattered: every number was right while the picture was of a different kind of object. The readout says in as many words that the point-mass model did not decide what they are. The same equations give the same waveform whatever the components are made of; identifying them is astrophysics, and for the neutron-star case the readout also names what the model leaves out — tidal deformation, which is how a real analysis tells them apart.',
      },
      {
      ...
      },
      {
        ...
        text: 'Moving any control used to restart the transport, so a student who paused on a feature and then nudged the playhead lost it immediately and concluded the pause button did not work. A control change now leaves the transport alone; only Play/pause and Replay change it.',
      },
      {
    ...
    misconceptions: [
      ...
      },
      {
        ...
        response:
          'It ends because the model is switched off there. "Where it stops" is the innermost stable circular orbit for that combined mass, which is the boundary of the assumption the whole waveform rests on - two bodies on a slowly shrinking circular orbit. Nothing past it has been calculated: there is no merger in these plots, no ringdown, and the last visible cycle is not the binary\u2019s last. "Time before merger" is of the same kind - the model extrapolating its own countdown to zero separation under an approximation it has already stopped satisfying. Screen 22 is where a merger actually appears, and it is the collaboration\u2019s numerical-relativity waveform rather than this model pushed past its boundary.',
      },
      {
