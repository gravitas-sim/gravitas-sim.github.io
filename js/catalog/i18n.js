// =============================================================================
// The catalog page's translator
// -----------------------------------------------------------------------------
// The same shape as js/observatory/i18n.js, for the same reason: /catalog/ is
// a small document page, and js/i18n/index.js would bring the application's
...
const CATALOGS = { en: EN_CATALOG, es: ES_CATALOG };
const DEFAULT = 'en';
let current = DEFAULT;
 ...
 * @param {string} id - A `cat.` message id
...
export function t(id, vars) {
  ...
  if (entry === undefined) {
    console.warn(`[catalog] no message for "${id}"`);
    return id;
  }
...
export function translatePage(root = document) {
  ...
  for (const el of root.querySelectorAll('[data-i18n-aria-roledescription]')) {
    el.setAttribute(
      'aria-roledescription',
      t(el.dataset.i18nAriaRoledescription)
    );
  }
  document.title = t('cat.doc.title');
}
