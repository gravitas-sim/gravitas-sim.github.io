...
// --- More than one world, without a second engine -----------------------------
// js/experiments/bench.js explains at length why the A/B bench runs its two
// arms sequentially: this module keeps the world in module-level arrays that
// every other module imports and mutates in place, there is one world per
// loaded module, and no constructor makes another. That is still true, and the
// refactor it describes is still not worth doing.
//
// What follows is the cheap two thirds of it. The arrays are exported *live
// bindings*, so reassigning the module-level variable is visible to every
// importer immediately - that is ordinary ES semantics and is what the engine
// already relies on when it filters a list and assigns the result back. So a
// second world does not need instance state threaded through render.js and
// ui.js and forty widgets. It needs the variables swapped around a call, which
// is exactly what withSeed() does for Math.random, and for the same reason:
// threading a handle through every call site is the expensive way to get a
// property that a save-and-restore already gives you.
//
// What this is NOT
// -----------------------------------------------------------------------------
// Not concurrency. Two worlds cannot run at once on one thread, and nothing
// here pretends otherwise: withWorld() is synchronous, and a world is only
// "current" while its call is on the stack. Interleaving means alternating, not
// overlapping. For genuine parallelism the answer is a Worker, which gets its
// own realm and therefore its own module instance for free - see
// spike/lyapunov/ - and needs none of this.
//
// Why these fields and not the nine the golden digests
// -----------------------------------------------------------------------------
// e2e/worldConstruction.spec.js hashes nine body lists, because those are the
// nine that contain bodies. A world is bigger than that. It is fourteen lists -
// the five extra ones are debris, particles, gwaves, gravity_ripples and
// accretion_disk_particles, which are effects rather than bodies and would
// otherwise leak from one world into the next - plus the tuning object, the id
// counter, the clock, and the conservation bookkeeping.
//
// The id counter is the one that would be missed. Ids come from a single
// counter and the golden pins them, so two worlds sharing it would produce a
// world whose bodies are numbered from wherever the other world stopped. The
// digest would change, every lesson that matches bodies by id across a rebuild
// would break, and nothing would say why.

/** How many worlds are on the stack. Diagnostic, as isSeeded() is for seeds. */
let worldDepth = 0;

/** The fourteen lists a world owns, by the name of the variable that holds it. */
const WORLD_LISTS = [
  'bh_list',
  'planets',
  'stars',
...
/**
 * Read the live world into a plain object.
 *
 * The arrays are taken by reference, not copied. A World is a handle on the
 * arrays that were current when it was made, and withWorld() puts those same
 * arrays back - so work done inside a world persists in it, which is what makes
 * alternating between two worlds useful rather than merely possible.
 *
 * @returns {Object} A World holding the current state
 */
function captureWorld() {
  return {
    ...
    accretion_disk_particles,
    physicsSettings,
    idCounter: PhysicsObject_id_counter,
    simulationTime,
    absorbedSpinAngularMomentum,
    discardedAbsorptionMomentum,
    conservationBaseline,
    totalsCache,
    totalsCachedAt,
    totalsCachedGeneration,
    cachedGravityDirty,
  };
}

/**
 * An empty world, with the current tuning copied into it.
 *
 * The settings are copied rather than shared: two worlds that pointed at one
 * settings object would have `updatePhysicsSettings` in one of them silently
 * retune the other, which is the failure this whole exercise exists to avoid.
 *
 * @returns {Object} A fresh World with no bodies in it
 */
function createWorld() {
  const w = {};
  for (const name of WORLD_LISTS) w[name] = [];
  w.physicsSettings = { ...physicsSettings };
  w.idCounter = 0;
  w.simulationTime = 0;
  w.absorbedSpinAngularMomentum = 0;
  w.discardedAbsorptionMomentum = { x: 0, y: 0 };
  w.conservationBaseline = null;
  w.totalsCache = null;
  w.totalsCachedAt = -Infinity;
  w.totalsCachedGeneration = -1;
  w.cachedGravityDirty = false;
  return w;
}

/** Install a World's fields into the module-level variables. */
function installWorld(w) {
  bh_list = w.bh_list;
  planets = w.planets;
  stars = w.stars;
  gas_giants = w.gas_giants;
  asteroids = w.asteroids;
  comets = w.comets;
  debris = w.debris;
  particles = w.particles;
  gwaves = w.gwaves;
  gravity_ripples = w.gravity_ripples;
  neutron_stars = w.neutron_stars;
  white_dwarfs = w.white_dwarfs;
  galaxies = w.galaxies;
  accretion_disk_particles = w.accretion_disk_particles;
  physicsSettings = w.physicsSettings;
  PhysicsObject_id_counter = w.idCounter;
  simulationTime = w.simulationTime;
  absorbedSpinAngularMomentum = w.absorbedSpinAngularMomentum;
  discardedAbsorptionMomentum = w.discardedAbsorptionMomentum;
  conservationBaseline = w.conservationBaseline;
  totalsCache = w.totalsCache;
  totalsCachedAt = w.totalsCachedAt;
  totalsCachedGeneration = w.totalsCachedGeneration;
  cachedGravityDirty = w.cachedGravityDirty;
}

/**
 * Run fn with `world` installed as the engine's world, then put the old one back.
 *
 * Modelled on withSeed() and sound for the same reason: JavaScript is
 * single-threaded, so nothing else can touch the engine while a synchronous
 * call is on the stack. The same caveat applies and is louder here - an `await`
 * inside the region would let the render loop step a world the caller thinks is
 * not installed - so an escaping thenable is reported rather than tolerated.
 *
 * The restore is in a finally block, so a throw inside fn cannot leave the page
 * pointing at a world the user cannot see. The world is written back on the way
 * out, so whatever fn built or moved stays in it.
 *
 * @param {Object} world - A World from createWorld() or captureWorld()
 * @param {Function} fn - Synchronous function to run against it
 * @returns {*} Whatever fn returns
 */
function withWorld(world, fn) {
  if (!world || typeof world !== 'object') {
    throw new TypeError('withWorld() needs a World from createWorld()');
  }
  const previous = captureWorld();
  installWorld(world);
  worldDepth++;
  try {
    const out = fn();
    if (out && typeof out.then === 'function') {
      console.warn(
        'withWorld() was given an async function. Only the synchronous part ' +
          'ran against this world; anything after the first await ran against ' +
          'whichever world was current by then.'
      );
    }
    return out;
  } finally {
    // Write back before restoring, so the World carries out what fn did to it.
    Object.assign(world, captureWorld());
    installWorld(previous);
    worldDepth--;
  }
}

/** @returns {boolean} True while a withWorld() region is on the stack */
const inWorld = () => worldDepth > 0;
