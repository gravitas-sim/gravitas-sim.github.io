// =============================================================================
// The figure builder's translator
// -----------------------------------------------------------------------------
// The same shape as js/submission/i18n.js, for the same reason: this is a small
// document page, and js/i18n/index.js would bring the application's whole
// catalog with it. It shares the `gravitas_locale` storage key, so a language
// chosen in the simulation is already chosen here, and reads only its own
// catalog pair. The function is `t`, so `npm run i18n:check` audits its ids.
//
// The page's static text carries `data-i18n` (text), `data-i18n-placeholder`
// and `data-i18n-aria-label` attributes naming a message, as the other
// document pages' markup does; translatePage() fills them.
// =============================================================================

import { EN_FIGURE } from '../i18n/en.figure.js';
import { ES_FIGURE } from '../i18n/es.figure.js';
...
const CATALOGS = { en: EN_FIGURE, es: ES_FIGURE };
const DEFAULT = 'en';
let current = DEFAULT;
 ...
 * @param {string} id - A `fig.` message id
...
export function t(id, vars) {
  ...
  if (entry === undefined) {
    console.warn(`[figure] no message for "${id}"`);
    return id;
  }
...
export function translatePage(root = document) {
  ...
  document.title = t('fig.doc.title');
}
